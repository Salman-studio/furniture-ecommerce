...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Check for duplicate subcategory name (excluding current subcategory)
    $sql_check = "SELECT subcategory_id FROM subcategories WHERE subcategory_name = ? AND subcategory_id != ?";
    ...
    if (mysqli_stmt_num_rows($stmt_check) > 0) {
        ...
        $sql = "UPDATE subcategories SET subcategory_name=?, description=?, category_id=? WHERE subcategory_id=?";
        $stmt = mysqli_prepare($conn, $sql);

        if ($stmt) {
            mysqli_stmt_bind_param($stmt, 'ssii', $name, $desc, $category_id, $id);
...
<div class="main-content">
    <h2 class="animate__animated animate__fadeInDown">Edit Subcategory</h2>
    ...
    <form method="POST" class="animate__animated animate__fadeInUp">
        <div class="form-group">
            ...
            <input name="name" id="name" value="<?= htmlspecialchars($sub['subcategory_name']) ?>" required class="form-control" />
        </div>
        <div class="form-group">
            <label for="description">Description</label>
            <textarea name="description" id="description" class="form-control" rows="5"><?= htmlspecialchars($sub['description']) ?></textarea>
        </div>
        <div class="form-group">
            ...
            <select name="category_id" id="category_id" class="form-select" required>
                ...
                <option value="<?= $cat['category_id'] ?>" <?= $cat['category_id'] == $sub['category_id'] ? 'selected' : '' ?>>
